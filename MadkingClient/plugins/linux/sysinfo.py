...
def collect():
    ...
    for key in filter_keys:
        try:
            # shell命令，midecode命令通过读取系统DMI表，显示服务器硬件和BIOS信息。
    ...
    for item in raw_ram_list:
        ...
        for i in item:
            ...
            if len(data) ==2:
                ...
                # 过滤Size关键字
                """
                [root@MiWiFi-R4C-srv ~]# dmidecode -t 17 | grep "Size"
                Size: 2048 MB
                Size: 128 MB
                Size: 8 MB
                ...
                if key == 'Size':
                    #print key ,v
                    if  v.strip() != "No Module Installed":  # 说明有内存
...
def osinfo():
    """系统信息:lsb_release命令用来查看当前系统的发行版信息"""
...
def cpuinfo():
    ...
    raw_data = {
        'cpu_model': "%s |grep 'model name' |head -1 " % base_cmd,  # modelname:CPU属于的名字及其编号、标称主频
        'cpu_count':  "%s |grep 'processor'|wc -l " % base_cmd,   # 查看逻辑CPU的个数
