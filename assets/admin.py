...
# ContentTypes组件：对当前项目中所有基于Django驱动的model提供了更高层次的抽象接口
...
class AssetAdmin(admin.ModelAdmin):
    list_display = ('id', 'asset_type', 'sn', 'name', 'manufactory',
                    'management_ip', 'idc', 'business_unit', 'admin', 'trade_date', 'status')
...
class NewAssetApprovalZoneAdmin(admin.ModelAdmin):
    # 定制右侧快速筛选(不能放多对多字段)
    list_display = ['sn', 'asset_type', 'manufactory', 'model', 'ram_size', 'cpu_model',
                    'date', 'approved', 'approved_by', 'approved_date']
    # 定制右侧快速筛选
    ...
    def approve_selected_objects(modeladmin, request, queryset):
        """批准选中对象"""
        # 获取用户选中的数据：request.POST.getlist()
